...
this.element().style.display='none';UserVoice.Element.removeClassName(this.htmlElement(),'dialog-open');this.Overlay.hide();UserVoice.onClose();},id:'uservoice-dialog',css_template:"\
   ...
   \
   a#uservoice-dialog-close { background-image: url(#{background_image_url}); }",element:function(){if(!document.getElementById(this.id)){var dummy=document.createElement('div');dummy.innerHTML='<div id="'+this.id+'" class="uservoice-component" style="display:none;">'+'<a href="#" onclick="UserVoice.Dialog.close(); return false;" id="'+this.id+'-close"><span style="display: none;">Close</span></a>'+'<div id="'+this.id+'-content"></div></div>';document.body.insertBefore(dummy.firstChild,document.body.firstChild);}
...
this.options.tab_string={cn:"ıœÕ˘Ê»",de:"Œ·‡ÚÌ‡ˇ",es:"Sugerencias",fi:"Palaute",fr:"Commentaires",ja:"Û√’Û¬„Û√-Û√…Û√–Û√√Û¬Ô",nl:"Feedback",pt_BR:"Coment+·rio"};this.options.text_url=UserVoice.Util.getAssetHost()+'/images/'+this.options.lang+'/feedback_tab_'+this.options.text_color+'.png';this.options.id=this.id;}}
