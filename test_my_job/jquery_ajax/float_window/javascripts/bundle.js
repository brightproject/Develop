...
$.extend($.facebox,{settings:{opacity:0,overlay:true,loadingImage:'/facebox/loading.gif',closeImage:'images/closelabel.gif',imageTypes:['png','jpg','jpeg','gif'],faceboxHtml:'\
...
$('#facebox .footer').hide()})})}
$.fn.spin=function(){return this.after('<img src="images/indicator.gif" id="spinner"/>')}
$.fn.stopSpin=function(){this.next().remove()
return this}
$(function(){$('.jshide').hide()
...
if(GitHub.hasWriteAccess&&GitHub.ignoredRepo){$('#fast_forward_button').show()}else if(GitHub.hasWriteAccess){$('#pull_request_button').show()}
...
if(link.length>0)
link.attr('href',url.replace(GitHub.currentRepoOwner,github_user).replace('/fork',''))}});$(function(){$('#downloads .delete').click(function(){if(confirm('Are you sure you want to delete this?')){$(this).hide().parents('form').append('deleting&hellip;').submit()}
return false})})
...
if(!url.match(/^http:\/\//)&&!url.match(/^svn:\/\//)){alert('Please enter a valid subversion url')
return false}
...
return false})
$('#master_branch').change(function(){var url=window.location.href.replace(/edit$/,'update')
...
var target='/'+GitHub.currentRepoOwner+'/'+GitHub.currentRepo+'/rebuild_gems'
...
$('.icons',next).html('<img src="images/indicator.gif" alt="Processing" />')
...
$('.icons',next).html('<img src="images/exclamation.png" alt="Failed" />')}else{$('#head-sha').text(data)
next.addClass('clean')
$('.date',next).html('applied')
$('.apply-status',next).attr('value','1')
$('.icons',next).html('<img src="images/commit.png" alt="Applied" />')}
...
$('#issues .display .loading div').corner({autoPad:false,tl:{},tr:{},bl:{radius:6},br:{radius:6}})})
...
this.primer.addChild(ownerCommitsLayer)}};$(function(){$('#signup_form').submit(function(){$('#signup_button').attr('disabled',true).val('Creating your GitHub account...')})});GitHub.CachedCommitDataPoller=function(){$.smartPoller(2000,function(retry){var sha,commit,msg,user,path=location.pathname,url=location.pathname.replace(/tree\/.+/,'cache/commits/')+GitHub.currentTreeSHA+'?path='+GitHub.currentPath+'&commit_sha='+GitHub.commitSHA,commitURL=location.pathname.replace(/tree\/.+/,'commit/')
